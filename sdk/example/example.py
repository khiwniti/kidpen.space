...
from kidpen.utils import print_stream
...
async def main():
    ...
    kidpen_client = kidpen.Kidpen(
        os.getenv("KIDPEN_API_KEY", "pk_xxx:sk_xxx"),
    ...
    if not agent_id:
        agent = await kidpen_client.Agent.create(
            name="Generic Agent",
        ...
        agent = await kidpen_client.Agent.get(agent_id)
        await agent.update(allowed_tools=["get_weather"])

    # Setup the thread
    thread_id = kv.get("thread_id")
    if not thread_id:
        thread = await kidpen_client.Thread.create()
        kv.set("thread_id", thread._thread_id)
    else:
        thread = await kidpen_client.Thread.get(thread_id)
