...
```
User request
  → Cloudflare DNS (api-eks.kidpen.com)
...
The EKS cluster is the main production target. Lightsail and ECS are still deployed to, but EKS handles the real traffic through `api-eks.kidpen.com`.
...
- **Uptime:** Set up HTTP monitors for `api-eks.kidpen.com`
