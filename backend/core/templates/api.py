...
class TemplateResponse(BaseModel):
    ...
    categories: List[str]
    is_public: bool
    is_kidpen_team: Optional[bool] = False
...
@router.get("/kidpen-all", response_model=MarketplaceTemplatesResponse)
async def get_all_kidpen_templates(
    ...
    try:
        ...
        try:
            ...
            error_str = f"Error of type {type(e).__name__}"
        logger.error(f"Error getting all Kidpen templates: {error_str}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_marketplace_templates(
    ...
    is_kidpen_team: Optional[bool] = Query(None, description="Filter for Kidpen team templates"),
