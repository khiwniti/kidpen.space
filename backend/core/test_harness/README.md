...
Comprehensive API-based E2E testing system for benchmarking the Kidpen agent system.
...
**Note**: The test harness automatically creates a test user (`testuser@kidpen.ai`) if it doesn't exist. No manual user setup required!
...
curl -X POST http://localhost:8000/v1/admin/test-harness/run \
  -H "X-Admin-Api-Key: $KIDPEN_ADMIN_API_KEY" \
...
- Test user (`testuser@kidpen.ai`) is automatically created with minimal permissions
