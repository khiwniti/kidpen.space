...
ON agents((metadata->>'is_kidpen_default')) 
...
- is_kidpen_default: boolean - Whether this is the official Suna default agent
...
CREATE OR REPLACE FUNCTION is_kidpen_default_agent(agent_row agents)
RETURNS boolean
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    RETURN COALESCE((agent_row.metadata->>'is_kidpen_default')::boolean, false);
END;
$$;
...
GRANT EXECUTE ON FUNCTION is_kidpen_default_agent(agents) TO authenticated, service_role;
...
CREATE POLICY agents_update_own ON agents
    ...
    USING (
        ...
        AND (
            NOT COALESCE((metadata->>'is_kidpen_default')::boolean, false)
            OR 
            (
                COALESCE((metadata->>'is_kidpen_default')::boolean, false) = true
            )
        )
    );
...
CREATE POLICY agents_delete_own ON agents
    ...
    USING (
        ...
        AND NOT COALESCE((metadata->>'is_kidpen_default')::boolean, false)
    );
