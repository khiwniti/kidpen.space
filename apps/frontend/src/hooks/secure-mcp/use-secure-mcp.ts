...
export function useMarketplaceTemplates(params?: {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      ...
      if (params?.is_kidpen_team !== undefined) searchParams.set('is_kidpen_team', params.is_kidpen_team.toString());
...
export function useKidpenTeamTemplates(options?: { enabled?: boolean }) {
  return useQuery({
    queryKey: ['secure-mcp', 'kidpen-templates-all'],
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      const supabase = createClient();
      const { data: { session } } = await supabase.auth.getSession();

      if (!session) {
        throw new Error('You must be logged in to view Kidpen templates');
      }

      const response = await fetch(`${API_URL}/templates/kidpen-all`, {
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
        },
      });
