...
/**
 * Check if a model ID corresponds to a Kidpen mode (Basic, Advanced, or Test)
 */
...
export function ModelProviderIcon({
  ...
  const iconMap: Record<ModelProvider, string> = {
    kidpen: '/kidpen-symbol.svg', // Kidpen modes use the Kidpen symbol
  ...
  // Special handling for Kidpen symbol - needs different invert behavior
  const isKidpen = provider === 'kidpen';
  ...
  return (
    ...
    >
      <Image
        ...
        className={cn(
          "object-contain",
          // Kidpen symbol: invert in dark mode (black symbol → white)
          // Other icons: invert in dark mode (black icons → white)
          isKidpen ? "dark:invert" : "dark:brightness-0 dark:invert"
        )}
