...
import { useKidpenTeamTemplates, useInstallTemplate } from '@/hooks/secure-mcp/use-secure-mcp';
...
export function CustomAgentsSection({ onAgentSelect }: CustomAgentsSectionProps) {
  ...
  const { data: templates, isLoading, error } = useKidpenTeamTemplates();
  ...
  const handleCardClick = (template: any) => {
    const marketplaceTemplate: MarketplaceTemplate = {
      ...
      is_kidpen_team: template.is_kidpen_team || false,
