...
const LoggedInMenu: React.FC<UnifiedConfigMenuProps> = memo(function LoggedInMenu({
    ...
    const placeholderSunaAgent = useMemo(() => ({
        agent_id: undefined,
        name: 'Kidpen',
        metadata: { is_kidpen_default: true }
    }), []);
    ...
    const renderAgentIcon = useCallback((agent: any, size: number = 32) => {
        if (!agent && (isLoading || sunaAgent)) {
            return <AgentAvatar isSunaDefault={true} agentName="Kidpen" size={size} className="flex-shrink-0 !border-0" />;
        }
    ...
    // Check if current agent is Kidpen (used in multiple places)
    const isKidpenAgent = !displayAgent?.name || displayAgent?.name === 'Kidpen';
                        ...
                        <div className="px-4 pb-2">
                            ...
                            >
                                ...
                                <span className="flex-1 truncate text-base font-medium text-left min-w-0">
                                    {isKidpenAgent ? 'Kidpen' : displayAgent?.name}
                                </span>
    ...
    }, [mobileSection, searchQuery, onAgentSelect, displayAgent, isLoading, placeholderSunaAgent, renderAgentIcon, selectedAgentId, AgentsList, CreateWorkerButton, WorkerSettingsButtons, isKidpenAgent]);
                            ...
                            {onAgentSelect && (
                                <>
                                    ...
                                    <div className="px-2 pb-2">
                                        <SpotlightCard className={cn("transition-colors cursor-pointer bg-transparent", isKidpenAgent ? "p-2" : "")}>
                                            <DropdownMenuSub>
                ...
                <span className="inline-block">
                    ...
                    >
                        <div className="flex items-center gap-2 min-w-0 max-w-[180px]">
                            ...
                            <span className="truncate text-sm font-medium">Kidpen</span>
                            <ChevronDown size={12} className="opacity-60 flex-shrink-0" />
                        </div>
                    </Button>
                </span>
