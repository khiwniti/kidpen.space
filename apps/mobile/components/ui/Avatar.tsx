...
interface AvatarProps extends ViewProps {
  ...
  /** Use Kidpen symbol instead of icon (for SUNA agent) */
  useKidpenSymbol?: boolean;
 ...
 * - Kidpen symbol: 50% of container (larger for brand recognition)
 * - Border: 1.5px solid
...
export function Avatar({
  ...
  const symbolSize = Math.round(size * 0.55); // 55% for Kidpen symbol (more prominent)
  ...
  const getDefaultColors = () => {
    ...
    // Kidpen symbol always uses solid black bg with white icon
    if (useKidpenSymbol) {
      return {
        bg: '#000000',
        icon: '#FFFFFF',
    ...
    <View 
      ...
      style={[
        { 
          ...
          borderWidth: useKidpenSymbol ? 0 : (showBorder ? 1.5 : 0),
