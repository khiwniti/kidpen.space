...
export function KidpenComputer({
  toolMessages,
  currentIndex,
  onNavigate,
  messages,
  ...
  log.log('[KidpenComputer] Render - toolMessages:', toolMessages.length, 'currentIndex:', currentIndex);
  log.log('[KidpenComputer] project:', project ? { id: project.id, name: project.name, hasSandbox: !!project.sandbox } : 'undefined');
  log.log('[KidpenComputer] sandboxId prop:', sandboxId);
  ...
  log.log('[KidpenComputer] currentPair:', currentPair ? 'has pair' : 'undefined');
  log.log('[KidpenComputer] currentPair.toolMessage:', currentPair?.toolMessage?.message_id || 'null');
  log.log('[KidpenComputer] currentPair.assistantMessage:', currentPair?.assistantMessage?.message_id || 'null');
  
  const { toolCall, toolResult, isSuccess, assistantTimestamp, toolTimestamp } = useMemo(() => {
    if (!currentPair?.toolMessage) {
      log.log('[KidpenComputer] No toolMessage in currentPair, returning null');
      return { toolCall: null, toolResult: null, isSuccess: false, assistantTimestamp: undefined, toolTimestamp: undefined };
    }
    log.log('[KidpenComputer] Calling extractToolCallAndResult');
