...
import KidpenSymbolIcon from '@/assets/brand/kidpen-symbol.svg';
...
/**
 * Check if a model ID corresponds to a Kidpen mode (Basic or Advanced)
 */
export function isKidpenMode(modelId: string): boolean {
  // New Kidpen registry IDs
  if (modelId === 'kidpen/basic' || modelId === 'kidpen/power' ||
      modelId === 'kidpen-basic' || modelId === 'kidpen-power') {
    return true;
  }
  ...
  // Legacy: Kidpen Advanced Mode (Sonnet 4.5)
...
export function getModelProvider(modelId: string): ModelProvider {
  // Check for Kidpen modes first
  if (isKidpenMode(modelId)) {
    return 'kidpen';
  }
  ...
  if (parts.length > 1) {
    ...
    if (['openai', 'anthropic', 'google', 'xai', 'moonshotai', 'bedrock', 'openrouter', 'kidpen'].includes(provider)) {
...
export function getModelProviderIcon(modelId: string): React.FC<SvgProps> {
  ...
  const iconMap: Record<ModelProvider, React.FC<SvgProps>> = {
    kidpen: KidpenSymbolIcon, // Kidpen modes use the Kidpen symbol
