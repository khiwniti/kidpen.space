...
export function useChat(): UseChatReturn {
  ...
  // Prioritizes kidpen/basic, then kidpen/power, then first accessible model
  const getDefaultModelId = useCallback((models: typeof accessibleModels): string | undefined => {
    // kidpen/basic should be first for free users since power is not accessible
    const basicModel = models.find(m => m.id === 'kidpen/basic');
    if (basicModel) return basicModel.id;

    const powerModel = models.find(m => m.id === 'kidpen/power');
  ...
  const VALID_MODEL_IDS = useMemo(() => new Set(['kidpen/basic', 'kidpen/power']), []);
  ...
  useEffect(() => {
    ...
    // CRITICAL: Only allow kidpen/basic or kidpen/power models
    // Other models (like kidpen/kimi-k2.5) are internal and should not be used
  ...
  const currentModel = useMemo(() => {
    ...
    // CRITICAL: Only use kidpen/basic or kidpen/power
    // Never use other models like kidpen/kimi-k2.5 (internal/legacy)
    ...
    // Fallback: prioritize kidpen/basic, then kidpen/power
    const basicModel = accessibleModels.find(m => m.id === 'kidpen/basic');
    if (basicModel) return basicModel.id;

    const powerModel = accessibleModels.find(m => m.id === 'kidpen/power');
    if (powerModel) return powerModel.id;
  ...
  const loadThread = useCallback((threadId: string) => {
    ...
    // Reset Kidpen Computer state when switching threads
    useKidpenComputerStore.getState().reset();
    log.log('[useChat] Reset Kidpen Computer state');
  ...
  const startNewChat = useCallback(() => {
    ...
    // Reset Kidpen Computer state when starting new chat
    useKidpenComputerStore.getState().reset();
    log.log('[useChat] Reset Kidpen Computer state for new chat');
